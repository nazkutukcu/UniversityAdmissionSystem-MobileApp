...
class Dersintibak extends StatefulWidget {
  const Dersintibak({Key? key}) : super(key: key);

  @override
  _Dersintibak createState() => _Dersintibak();
}

class _Dersintibak extends State<Dersintibak> {
            ...
            child: Column(
              children: <Widget>[
                ...
                Container(
                  ...
                  child: TextFormField(
                    decoration: InputDecoration(
                      labelText: 'Muaf olmak istenilen ders',
                      labelStyle: TextStyle(color: Colors.black),
                    ),
                    validator: (val) => val!.isEmpty ? 'muaf olmak istediğiniz dersi giriniz' : null,
                    onChanged: (val) {
                      setState(() => muafDers = val);
                    },
                  ),
                ),
                SizedBox(
...
Container(
                  child: MaterialButton(
                    ...
                    height: 60,
                    onPressed: () async {
                      //burada fonksiyon calisir           
                      if (formKey.currentState!.validate()&&fileController=='fileSelected') {
                      dynamic result = await storage.setApplyDataIntibak(
                          name,
                          ogrenciNo,
                          kimlikNo,
                          telNo,
